...
export const MobileNav = () => {
    ...
    $(window).on('load resize orientationchange touchstart pointerdown', function() {
        setTimeout(function() {
            if( $(window).width() < 1023 ) {
                $('.header').find('.search-box').addClass('sm');
            } else {
                $('.header').find('.search-box').removeClass('sm');

                $('body').hasClass('nav-active') ? $('body').removeClass('nav-active') : false;
            }
        }, 100)
    });
}
